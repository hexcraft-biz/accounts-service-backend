...
import (
	...
	"github.com/hexcraft-biz/accounts-service-backend/misc"
	"github.com/hexcraft-biz/accounts-service-backend/models"
...
func (ctrl *Auth) SignUpEmailConfirm() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		subject := ctrl.Config.Env.SignupEmailSubject
		...
		tmpl.Execute(&tpl, struct {
			...
			ctrl.Config.Env.SignupEmailContent,
			realVerifyPageURI,
			ctrl.Config.Env.SignupEmailLinkText,
		})

		body := tpl.String()
		email.SendHTML(ctrl.Config.Env.SMTPSender, to, subject, body)
...
func (ctrl *Auth) ForgetPwdConfirm() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		to := []string{params.Email}
		subject := ctrl.Config.Env.ForgetPwdEmailSubject
		...
		tmpl.Execute(&tpl, struct {
			...
			ctrl.Config.Env.ForgetPwdEmailContent,
			realVerifyPageURI,
			ctrl.Config.Env.ForgetPwdEmailLinkText,
		})
